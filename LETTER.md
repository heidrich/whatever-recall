...
Picture a player stuck on a quest. The user asks, "What's wrong?" The Game Master answers: *"Ah, I see you're stuck on this quest. The way I see it, you've either talked to the wrong NPC, or the quest isn't actually complete yet — did you check your quest log?"* Why two answers and not twenty? Because the GM knows the system inside out. The user's problem is real, but I — the GM — know it can only be one of two things. So I hand them the only two answers that can possibly be right, and the guessing is over before it began.
